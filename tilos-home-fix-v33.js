...
function utilities(){
 ...
 let nav=home.querySelector('.vh-utility-actions');if(!nav){nav=document.createElement('nav');nav.className='vh-utility-actions';nav.setAttribute('aria-label','Tilos help and assessor profile');nav.innerHTML='<button type="button" data-v33-help>Help / Tilos</button><button type="button" data-v33-profile>Edit profile</button>';home.appendChild(nav)}
}
...
if(!document.querySelector('#v33-home-style')){const s=document.createElement('style');s.id='v33-home-style';s.textContent='#settingsButton{display:none!important}.vh-utility-actions{display:flex;gap:16px;justify-content:flex-start;margin-top:-4px;padding:0 3px 6px}.vh-utility-actions button{border:0;background:transparent;padding:2px 0;color:#6f797e;font:inherit;font-size:11px;text-decoration:none}.vh-utility-actions button:active{color:#2583a7}';document.head.appendChild(s)}
